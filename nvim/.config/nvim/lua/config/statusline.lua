...
local palette = {
  ...
  dark_cyan = "#2a303c",
  dark_gray = "#1d2021",
  dark_sienna = "#363a4f", -- surface 0
  deep_teal = "#2e4954",
  forest_mist = "#35403b",
  light_gray = "#b8c0e0",  -- sub txt 0
  light_green = "#8bd5ca", -- teal
...
hi("StatusDiffAdd", { guibg = palette.dark_sienna, guifg = palette.light_green, gui = "bold" }) -- guibg was none
-- hi("StatusDiffAdd", { guibg = palette.light_green, guifg = palette.dark_sienna, gui = "bold" }) -- guibg was none
hi("StatusDiffChange", { guibg = palette.dark_sienna, guifg = palette.yellow, gui = "bold" })   -- guibg was none, guifg was dark_sienna
-- hi("StatusDiffChange", { guibg = palette.yellow, guifg = palette.dark_sienna, gui = "bold" })   -- guibg was none
hi("StatusDiffDelete", { guibg = palette.dark_sienna, guifg = palette.red, gui = "bold" })      -- guibg was none, guifg was dark_sienna
-- hi("StatusDiffDelete", { guibg = palette.red, guifg = palette.dark_sienna, gui = "bold" })      -- guibg was none

hi("StatusGitRepoName", { guibg = palette.dark_sienna, guifg = palette.overlay1 }) -- guibg was none, guifg was dark_sienna
hi("StatusGitBranch", { guibg = palette.dark_sienna, guifg = NONE })               -- guibg was none, guifg was dark_sienna

--file
hi("StatusFile", { guibg = NONE, guifg = NONE, gui = "bold" })
hi("StatusFilePath", { guibg = NONE, guifg = palette.overlay1 }) -- light_gray
...
hi("StatusLocation", { guibg = palette.mauve, guifg = palette.dark_gray })
hi("StatusPercent", { guibg = palette.lavender, guifg = palette.dark_gray, gui = "bold" })
...
local function get_git_branch()
  ...
  if gs and gs.root then
    ...
    return "%#StatusGitRepoName#" .. repo_name .. "/" .. "%#StatusGitBranch#" .. branch
  end
...
-- Truncate path by keeping first char of intermediate directories
-- e.g., "some/very/long/path/config" -> "some/v/l/p/config"
local function truncate_path(path, max_len)
  ...
  if #parts <= 2 then
    return path -- Keep first and last fully if only 2 parts
  end

  -- Keep first directory and last directory (direct parent) fully
...
function M.build()
  ...
  if path ~= "" then
    path = truncate_path(path, 20) -- Adjust max_len as needed
    st = st .. "%#StatusFilePath# " .. path .. "/" .. "%#StatusFileToNorm#"
  end

  local filename = fn.expand("%:t")
  if filename ~= "" then
    st = st .. "%#StatusFile#" .. filename .. " " .. (vim.bo.modified and "" or "") .. "%#StatusFileToNorm#"
  end
